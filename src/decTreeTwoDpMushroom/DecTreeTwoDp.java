...
public class DecTreeTwoDp extends AbstractClassifier{
	...
	private void partitionTwo(Node node, Set<Attribute> Attrs){
        ...
        int initialdepth = 1; 
		randomSplitTwoTree(node,Attrs,initialdepth);
	...
	private void randomSplitTwoTree(Node node, Set<Attribute> Attrs, int time){
		...
		for(int i=0; i < parts.length; i++){
			...
			randomSplitTwoTree(children[i], Attrs, time+1);     
		}
